...
def gen_assert_one(A,au,reg1,reg1_val,reg2,reg2_val,prev_value):
        ...
        A("msub {},{}".format(reg1,reg2))
        next_value = prev_value - result;
        next_value = numutil.sign64(next_value)
...
def my_gen1(A,au):
    ...
    previous_value = gen_partial(A, au, lambda : numutil.u32(), lambda : numutil.u32(),previous_value,time = 3)
    au.check_and_exit()
r.gen(my_gen1)
