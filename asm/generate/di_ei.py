...
##############################################################""")
    ...
    for x in range(1000):
        retrive_reg, rt = regutil.get_random(k=2)
        A(f"mfc0 {retrive_reg},$12")
        using_di = numutil.below(2) > 0
        if using_di:
            A(f"di {rt}")
        else:
            A(f"ei  {rt}")
        au.assert_equal(retrive_reg, rt)
        A(f"mfc0 {retrive_reg},$12")
        A("andi {0},{0},1".format(retrive_reg))
        if using_di:
            au.assert_equal(retrive_reg, 0)
        else:
            au.assert_equal(retrive_reg, 1)
